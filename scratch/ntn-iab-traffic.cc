 ...
 *                 Integrating NTNs & Multibeam with IAB derived from ns3-mmwave-iab, ns3-ntn and ns3-mmwave-hbf
 *                  
 *             
 */
 ...
 Ptr<OutputStreamWrapper> g_genericThroughputLogUl;
 
 // Track sinks per UE across protocols, to compute throughput using GetTotalRx
 std::map<uint32_t, Ptr<PacketSink>> g_genericSinks;
 // Track UL sinks per UE on remote host
 std::map<uint32_t, Ptr<PacketSink>> g_genericSinksUl;
 ...
 void NetworkFeedbackLoop(uint32_t ueIndex)
 {
     ...
     try
     {
         ...
         if (g_adaptiveFeedbackLog)
         {
             ...
             const uint64_t bytesDiff = metrics.bytesReceived - prevBytesReceived;
             double throughputMBps = (windowInSeconds > 0.0)
                                     ? (static_cast<double>(bytesDiff) / windowInSeconds) / (1024.0 * 1024.0)
                                     : 0.0;
 ...
 void InitializeAdaptiveFeedback(uint32_t ueIndex, Ptr<TrafficGenerator3gppGenericVideo> videoTg, Ptr<PacketSink> sink)
 {
     ...
     Simulator::Schedule(Seconds(0.6), &NetworkFeedbackLoop, ueIndex);
 ...
 {
     static std::map<uint32_t, std::pair<uint64_t, Time>> last; // ueIndex -> (bytes, time)
     Time now = Simulator::Now();
     for (const auto &kv : g_genericSinks)
     {
         uint32_t ue = kv.first;
         Ptr<PacketSink> sink = kv.second;
         if (!sink)
         {
             continue;
         }
         uint64_t bytesNow = sink->GetTotalRx();
         uint64_t bytesPrev = 0;
         Time timePrev = now;
         auto it = last.find(ue);
         if (it != last.end())
         {
             bytesPrev = it->second.first;
             timePrev = it->second.second;
         }
         double dt = (now - timePrev).GetSeconds();
         if (dt > 0)
         {
             double thrBytesPerSec = static_cast<double>(bytesNow - bytesPrev) / dt;
             if (g_genericThroughputLog)
             {
                 *g_genericThroughputLog->GetStream()
                     << "Time: " << now.GetSeconds()
                     << ", UE: " << ue
                     << ", Bytes: " << bytesPrev << " -> " << bytesNow
                     << ", Throughput: " << thrBytesPerSec / (1024.0 * 1024.0) << " MB/s"
                     << std::endl;
             }
             else
             {
                 NS_LOG_UNCOND("[GenericThroughput] t=" << now.GetSeconds() << "s UE=" << ue
                                 << " thr=" << thrBytesPerSec / (1024.0 * 1024.0) << " MB/s");
             }
         }
         last[ue] = {bytesNow, now};
     }
     // Re-schedule at feedback interval (reuse g_feedbackInterval for cadence)
     Simulator::Schedule(Seconds(g_feedbackInterval), &GenericThroughputLoop);
 }
 
 // Periodic throughput calculator for UL gaming (or any UL using PacketSink on remote host)
 static void GenericThroughputLoopUl()
 {
     static std::map<uint32_t, std::pair<uint64_t, Time>> last; // ueIndex -> (bytes, time)
     Time now = Simulator::Now();
     for (const auto &kv : g_genericSinksUl)
     {
         uint32_t ue = kv.first;
         Ptr<PacketSink> sink = kv.second;
         if (!sink)
         ...
         {
             double thrBytesPerSec = static_cast<double>(bytesNow - bytesPrev) / dt;
             if (g_genericThroughputLogUl)
             {
                 *g_genericThroughputLogUl->GetStream()
                     << "Time: " << now.GetSeconds()
                     << ", UE: " << ue
                     << ", Bytes: " << bytesPrev << " -> " << bytesNow
                     << ", Throughput: " << thrBytesPerSec / (1024.0 * 1024.0) << " MB/s"
             ...
             {
                 NS_LOG_UNCOND("[GenericThroughput UL] t=" << now.GetSeconds() << "s UE=" << ue
                                 << " thr=" << thrBytesPerSec / (1024.0 * 1024.0) << " MB/s");
             }
         }
         last[ue] = {bytesNow, now};
     }
     Simulator::Schedule(Seconds(g_feedbackInterval), &GenericThroughputLoopUl);
 }
 ...
 // Helper function to get numeric traffic code as string
 std::string
 GetTrafficTypeNumber(const std::string& trafficType)
 {
     if (trafficType == "ftp" || trafficType == "0")
         return "0";
     else if (trafficType == "video" || trafficType == "1")
 ...
 main (int argc, char *argv[])
 {
  ...
  //  LogComponentEnable("MmWaveUeMac", LOG_LEVEL_ALL);
   ...
   uint32_t appDuration = 20;  // Application duration in seconds
 ...
 //   Config::SetDefault ("ns3::MmWavePaddedHbfMacScheduler::CqiTimerThreshold", UintegerValue (100));
   ...
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::aParamPacketSizeDl", UintegerValue(900));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::bParamPacketSizeDl", DoubleValue(20.0));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::aParamPacketArrivalDl", DoubleValue(10.0));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::bParamPacketArrivalDl", DoubleValue(2.0));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::PacketArrivalUl", UintegerValue(5));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::InitialPacketArrivalMin", UintegerValue(0));
   Config::SetDefault("ns3::TrafficGeneratorNgmnGaming::InitialPacketArrivalMax", UintegerValue(5));
 
   // NGMN VoIP: maximize activity (note: codec rate is ~12.2 kbps; throughput won't exceed that)
   Config::SetDefault("ns3::TrafficGeneratorNgmnVoip::VoiceActivityFactor", DoubleValue(0.95));
   Config::SetDefault("ns3::TrafficGeneratorNgmnVoip::EncoderFrameLength", UintegerValue(10));
   Config::SetDefault("ns3::TrafficGeneratorNgmnVoip::SIDPeriodicity", UintegerValue(80));
 
   // 3GPP Audio/Data: raise data rate and increase cadence
 ...
 //  Config::SetDefault("ns3::MmWavePhyMacCommon::NumEnbLayers", UintegerValue(2));
   ...
   Vector posIab1 = Vector(xMax / 2.0 + 150 , yMax / 2.0, iabHeight);        // Mid
   ...
   double ueX = xMax/2.0+200;  // Same X coordinate as eNB
   double ueY = yMax/2.0;  // 100 meters north of eNB
 ...
 // uint32_t extraUes = totalUes % clusterCount;              // Remaining UEs to distribute
   ...
   if (useTrafficGenerators)
   {
     ...
     for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
     {
       ...
       // Use PacketSink for both UDP and TCP to enable unified throughput measurement
       PacketSinkHelper dlPacketSinkHelper(ns3TransportProtocol,
                                           InetSocketAddress(Ipv4Address::GetAny(), dlPort));
       serverApps.Add(dlPacketSinkHelper.Install(ueNodes.Get(u)));
       NS_LOG_UNCOND("UE " << u << ": PacketSink installed on port " << dlPort << " for " << transportProtocol << " traffic");
       
       // Install DL traffic generator on remote host
         ...
         if (tg)
         {
           ...
           if (videoTg)
           {
             ...
             if (sink)
             {
               ...
               NS_LOG_UNCOND("Initializing adaptive feedback for UE " << u);
             }
             else
             {
     ...
     // If gaming, also install UL gaming flows (UE -> remoteHost)
     if (trafficType == "gaming" || trafficType == "2")
     {
       NS_LOG_UNCOND("Installing UL gaming traffic (UE -> remoteHost)...");
       std::string ns3TransportProtocol = GetNs3TransportProtocol(transportProtocol);
       // UL server on remote host to receive from UEs
       PacketSinkHelper ulSinkHelper(ns3TransportProtocol,
                                     InetSocketAddress(Ipv4Address::GetAny(), ulPort));
       ulServerApps.Add(ulSinkHelper.Install(remoteHost));
       NS_LOG_UNCOND("RemoteHost: PacketSink installed on UL port " << ulPort);
       for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
       {
         // UL generator on each UE, IsDownlink=false
         TrafficGeneratorHelper ulTgHelper(
           ns3TransportProtocol,
           InetSocketAddress(internetIpIfaces.GetAddress(1), ulPort),
           TrafficGeneratorNgmnGaming::GetTypeId()
         );
         ulTgHelper.SetAttribute("IsDownlink", BooleanValue(false));
         ApplicationContainer ulApp = ulTgHelper.Install(ueNodes.Get(u));
         ulClientApps.Add(ulApp);
         NS_LOG_UNCOND("UE " << u << ": UL gaming traffic generator installed to remoteHost:" << internetIpIfaces.GetAddress(1) << ":" << ulPort);
       }
     }
   ...
   if (useTrafficGenerators && GetNs3TransportProtocol(transportProtocol) == "ns3::TcpSocketFactory")
   {
     ...
     std::string trafficNum = GetTrafficTypeNumber(trafficType);
     Ptr<OutputStreamWrapper> tcpRxStream = ascii.CreateFileStream(std::string("TcpPacketRx_") + trafficNum + ".txt");
     ...
     *tcpRxStream->GetStream() << "Time(s)\tSource_IP\tSource_Port\tPacket_Size\tProtocol\tNode_ID\tUE_Index" << std::endl;
     ...
     NS_LOG_UNCOND("TCP tracing enabled. Output will be written to 'TcpPacketRx_" << trafficNum << ".txt'");
     
     // Create adaptive feedback logging file for 3GPP video traffic
     if (trafficType == "4" || trafficType == "3gpp-video")
     {
       std::string trafficNum = GetTrafficTypeNumber(trafficType);
       g_adaptiveFeedbackLog = ascii.CreateFileStream(std::string("AdaptiveFeedback_") + trafficNum + ".txt");
       *g_adaptiveFeedbackLog->GetStream() << "# Adaptive Feedback Log - Format: Time, UE index, Packet Loss, Data received changes, FPS changes, DataRate changes" << std::endl;
       NS_LOG_UNCOND("Adaptive feedback logging enabled. Output will be written to 'AdaptiveFeedback_" << trafficNum << ".txt'");
     }
   }
   else
   {
   ...
   // Create generic throughput log file with traffic suffix
   AsciiTraceHelper ascii;
   std::string trafficNum = GetTrafficTypeNumber(trafficType);
   g_genericThroughputLog = ascii.CreateFileStream(std::string("GenericThroughput_") + trafficNum + ".txt");
   *g_genericThroughputLog->GetStream() << "# DL Time, UE, Bytes(prev->now), Throughput(MB/s)" << std::endl;
 
   // Create separate UL throughput file
   g_genericThroughputLogUl = ascii.CreateFileStream(std::string("GenericThroughput_UL_") + trafficNum + ".txt");
   *g_genericThroughputLogUl->GetStream() << "# UL Time, UE, Bytes(prev->now), Throughput(MB/s)" << std::endl;
   ...
   for (uint32_t i = 0; i < serverApps.GetN(); i++)
   {
     ...
     if (sink)
     {
       g_genericSinks[i] = sink;
     }
   }
 
   // Track UL sinks on remote host for throughput logging
   for (uint32_t i = 0; i < ulServerApps.GetN(); i++)
   {
     Ptr<Application> app = ulServerApps.Get(i);
     Ptr<PacketSink> sink = DynamicCast<PacketSink>(app);
     if (sink)
     {
       g_genericSinksUl[i] = sink;
     }
   }
 
   // Kick off loop a little after start to allow initial traffic
   Simulator::Schedule(Seconds(0.5), &GenericThroughputLoop);
   NS_LOG_UNCOND("Generic throughput logging (MB/s) enabled for non-type-4 traffic. Output: 'GenericThroughput_" << trafficNum << ".txt'");
 
   // Kick off UL throughput loop as well
   Simulator::Schedule(Seconds(0.5), &GenericThroughputLoopUl);
