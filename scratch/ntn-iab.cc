...
//  Config::SetDefault("ns3::MmWavePhyMacCommon::NumEnbLayers", UintegerValue(2));
  ...
  NS_LOG_DEBUG("=== BANDWIDTH VERIFICATION ===");
  NS_LOG_DEBUG("ChunkWidth: " << phyMacConfig->GetChunkWidth() / 1e6 << " MHz");
  NS_LOG_DEBUG("ChunkPerRB: " << phyMacConfig->GetNumChunkPerRb());
  NS_LOG_DEBUG("ResourceBlockNum: " << phyMacConfig->GetNumRb());
  NS_LOG_DEBUG("Total Bandwidth: " << (phyMacConfig->GetChunkWidth() * phyMacConfig->GetNumChunkPerRb() * phyMacConfig->GetNumRb()) / 1e6 << " MHz");
  NS_LOG_DEBUG("Center Frequency: " << phyMacConfig->GetCenterFrequency() / 1e9 << " GHz");
  NS_LOG_DEBUG("================================");
  ...
  cmd.Parse(argc, argv);
  NS_LOG_DEBUG("Throughput: "<<throughput<<" Inter-packet interval: "<<interPacketInterval);
...
// uint32_t extraUes = totalUes % clusterCount;              // Remaining UEs to distribute
  ...
  // Print coordinates of UEs, ENBs, and IAB nodes using NS_LOG_DEBUG
  NS_LOG_DEBUG("\n=== Node Coordinates ===");
  for (NodeList::Iterator it = NodeList::Begin (); it != NodeList::End (); ++it)
  {
    Ptr<Node> node = *it;
