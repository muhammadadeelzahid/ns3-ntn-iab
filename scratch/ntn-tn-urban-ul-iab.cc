...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("MmWaveNtnUrbanUlIab");
...
main (int argc, char *argv[])
{
  ...
  uint32_t interPacketInterval = 200;   // change this to 50 for 224 Mbps 100 for 112 mbps
  ...
  Config::SetDefault("ns3::MmWave3gppPropagationLossModel::NTNScenario", StringValue("Urban"));
  ...
  offsetX->SetAttribute("Min", DoubleValue(-1*max_distance));
  offsetX->SetAttribute("Max", DoubleValue(max_distance));
  offsetY->SetAttribute("Min", DoubleValue(-1*max_distance));
  offsetY->SetAttribute("Max", DoubleValue(max_distance));
  ...
  double buildingWidthX = 60.0;
  double buildingWidthY = 60.0;
  double buildingHeight = 10.0;

  // Calculate how many buildings fit in each direction
  int numBuildingsX = static_cast<int>((xMax + streetWidth) / (buildingWidthX + streetWidth));
  int numBuildingsY = static_cast<int>((yMax + streetWidth) / (buildingWidthY + streetWidth));

  double gridWidth = numBuildingsX * buildingWidthX + (numBuildingsX - 1) * streetWidth;
  double gridHeight = numBuildingsY * buildingWidthY + (numBuildingsY - 1) * streetWidth;

  double gridStartX = (xMax - gridWidth) / 2.0;
  double gridStartY = (yMax - gridHeight) / 2.0;

  for (int i = 0; i < numBuildingsX; ++i)
  {
      for (int j = 0; j < numBuildingsY; ++j)
      {
          double minX = gridStartX + i * (buildingWidthX + streetWidth);
          double minY = gridStartY + j * (buildingWidthY + streetWidth);
          double maxX = minX + buildingWidthX;
          double maxY = minY + buildingWidthY;

          Ptr<Building> building = Create<Building>();
          building->SetBoundaries(Box(minX, maxX, minY, maxY, 0.0, buildingHeight));
  ...
  for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
  {
      ...
      ulClient.SetAttribute("MaxPackets", UintegerValue(1523));
