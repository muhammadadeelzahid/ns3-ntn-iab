...
void 
EpcMmeApplication::AddIab (uint16_t gci, uint64_t imsi)
{
  NS_LOG_FUNCTION (this << gci << imsi);
  Ptr<EnbInfo> enbInfo = Create<EnbInfo> ();
  enbInfo->gci = gci;
  enbInfo->iab = true;
  enbInfo->imsi = imsi;
  ...
  m_iabCellIdByImsi[imsi] = gci;

  // also list the IAB node as UE
  AddUe(imsi);
}
...
EpcMmeApplication::AddUe (uint64_t imsi)
{
  ...
  if(m_iabCellIdByImsi.find(imsi) != m_iabCellIdByImsi.end())
  {
    ueInfo->iab = true;
  }
...
EpcMmeApplication::DoInitialUeMessage (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, uint64_t imsi, uint16_t gci)
{
  ...
  while(gnbIt->second->iab)
  {
    // the donor node is an IAB node
    // set a flag and the cellId of the first wired gNB
    it->second->donorWireless = true;
    
    // get the imsi of the donor
    uint64_t donorImsi = gnbIt->second->imsi;

    NS_LOG_DEBUG("The donorImsi is " << donorImsi);

    // get the UeInfo of the donor
    auto donorIt = m_ueInfoMap.find(donorImsi);
    uint16_t donorParentId = donorIt->second->cellId;

    NS_LOG_DEBUG("The parent of the donor has cellId " << donorParentId);

    gnbIt = m_enbInfoMap.find(donorParentId);
    NS_ASSERT(gnbIt != m_enbInfoMap.end());
  }
  it->second->wiredId = gnbIt->second->gci; // i.e., the cellId of the first wired or directly the cellId gci (if the loop is never executed)
  NS_LOG_DEBUG("The cellId of the first wired for UE imsi " << imsi << " is " << it->second->wiredId << " iab dev? " << it->second->iab);

  EpcS11SapSgw::CreateSessionRequestMessage msg;
  msg.imsi = imsi;
  msg.uli.gci = it->second->wiredId; // give to epc-sgw-pgw-application the info on the first WIRED base station, not on the actual base station to which the UE is connected
...
EpcMmeApplication::DoCreateSessionResponse (EpcS11SapMme::CreateSessionResponseMessage msg)
{
  ...
  uint16_t cellId = it->second->wiredId; // cellId of the first wired!
  bool iab = it->second->iab; // is this an IAB dev?
  
  uint16_t enbUeS1Id = it->second->enbUeS1Id; // this is the rnti in the cell
  uint64_t mmeUeS1Id = it->second->mmeUeS1Id; // this is the imsi - again
  ...
  m_s1apSapMmeProvider->SendInitialContextSetupRequest (mmeUeS1Id, enbUeS1Id, erabToBeSetupList, cellId, iab);
}
