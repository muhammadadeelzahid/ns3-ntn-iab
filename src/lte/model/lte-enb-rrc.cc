...
UeManager::DoInitialize ()
{
  ...
  m_needIabPhyMacConfiguration = false;
  m_iab = false;
}
...
void
UeManager::SetupDataRadioBearer (EpsBearer bearer, uint8_t bearerId, uint32_t gtpTeid, Ipv4Address transportLayerAddress, bool iab) 
{
  ...
  if (rlcTypeId != LteRlcSm::GetTypeId ())
    {
      ...
      if(iab) // set a larger RLC queue size
      {
        rlc->SetMaxTxBufferSize(20*rlc->GetMaxTxBufferSize()); //TODOIAB parameter-attribute for this!
      }
    }
...
UeManager::RecvRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg)
{
  ...
  switch (m_state)
    ...
    case CONNECTION_RECONFIGURATION:
      ...
      if(m_needIabPhyMacConfiguration)
      {
         // configure MAC (and scheduler)
        LteEnbCmacSapProvider::UeConfig req;
        req.m_rnti = m_rnti;
...
LteEnbRrc::DoRecvHandoverRequest (EpcX2SapUser::HandoverRequestParams req)
{
  ...
  for (std::vector <EpcX2Sap::ErabToBeSetupItem>::iterator it = req.bearers.begin ();
       ...
       ++it)
    {
      ueManager->SetupDataRadioBearer (it->erabLevelQosParameters, it->erabId, it->gtpTeid, it->transportLayerAddress, false); // TODOIAB during HO check the IAB flag!!
