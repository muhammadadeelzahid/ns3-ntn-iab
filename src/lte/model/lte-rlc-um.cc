...
void
LteRlcUm::SetMaxTxBufferSize(uint32_t maxTxBuffersize)
{
  m_maxTxBufferSize = maxTxBuffersize;
}

uint32_t
LteRlcUm::GetMaxTxBufferSize()
{
...
LteRlcUm::DoNotifyTxOpportunity (LteMacSapUser::TxOpportunityParameters txOpParams)
{
  ...
  if (m_lcid == 3)
  {
    NS_LOG_DEBUG ("*** RLC UM LCID 3 TX OPPORTUNITY: RNTI=" << m_rnti << " LCID=" << (uint32_t)m_lcid << " bytes=" << txOpParams.bytes << " at time " << Simulator::Now().GetSeconds() << "s");
  }
