...
void
MmWaveBeamformingModel::SetSpectrumPropagationLossModel (Ptr<MmWave3gppChannel> spectrumPropagationLossModel)
{
  NS_LOG_FUNCTION (this);
  m_3gppChannel = spectrumPropagationLossModel;
}
...
MmWaveDftBeamforming::DoDesignBeamformingVectorForDevice (Ptr<NetDevice> otherDevice, uint8_t layerInd)
{
  NS_LOG_DEBUG("MmWaveDftBeamforming::DoDesignBeamformingVectorForDevice");
  ...
  if ( notFound || update )
    {
      ...
      AntennaArrayBasicModel::BeamformingVector newBfStruct = DoDesignBeamformingVectorForDevice (otherDevice,layerInd);
...
MmWaveFFTCodebookBeamforming::CheckBfCacheExpiration(Ptr<NetDevice> otherDevice, Ptr<BFVectorCacheEntry> pCacheValue)
{
  ...
  //Ptr<ThreeGppSpectrumPropagationLossModel> casted3GPPchan = DynamicCast<ThreeGppSpectrumPropagationLossModel>( m_3gppChannel );
...
AntennaArrayBasicModel::BeamformingVector
MmWaveFFTCodebookBeamforming::DoDesignBeamformingVectorForDevice (Ptr<NetDevice> otherDevice, uint8_t layerInd)
{
  // the functions that we need from casted3GPPchan are actually in mmwave-3gpp-channel
  //Ptr<ThreeGppSpectrumPropagationLossModel> casted3GPPchan = DynamicCast<ThreeGppSpectrumPropagationLossModel>( m_3gppChannel );
  Ptr<MmWave3gppChannel> casted3GPPchan = m_3gppChannel;
...
{
  NS_LOG_DEBUG("MmWaveMMSEBeamforming::MmseCholesky");
...
{
  NS_LOG_DEBUG("MmWaveMMSEBeamforming::MmseSolve");
...
{
  NS_LOG_DEBUG("MmWaveMMSEBeamforming::GetBfCachesInSlotBundle");
  ...
  for (std::vector< Ptr<NetDevice> >::iterator itDev = vOtherDevs.begin() ; itDev != vOtherDevs.end() ; itDev++ )
    {//retrieve the analog BF cache items, while making sure that the analog BF part is up to date
      ...
      if ( notFound | update ){
          ...
          DoDesignBeamformingVectorForDevice ( (*itDev ),0 ); //we do not use the output value directly in this call
          // to do adeel verify this layer index is correct
...
{
  NS_LOG_DEBUG("MmWaveMMSEBeamforming::SetBeamformingVectorForSlotBundle");  
