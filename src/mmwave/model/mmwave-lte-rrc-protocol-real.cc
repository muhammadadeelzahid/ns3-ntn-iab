...
MmWaveLteUeRrcProtocolReal::SetEnbRrcSapProvider ()
{
          ...
          if (enbDev == 0)
            {
              ...
              if(mmWaveEnbDev == 0)
              {
                iabDev = node->GetDevice (j)->GetObject <MmWaveIabNetDevice>();
                if(iabDev == 0)
                {
                  continue;                
                } 
  ...
  {
    m_enbRrcSapProvider = iabDev->GetAccessRrc ()->GetLteEnbRrcSapProvider();
    Ptr<MmWaveLteEnbRrcProtocolReal> enbRrcProtocolReal = iabDev->GetAccessRrc ()->GetObject<MmWaveLteEnbRrcProtocolReal> ();
    enbRrcProtocolReal->SetUeRrcSapProvider (m_rnti, m_ueRrcSapProvider);    
  }
  else
  {
    NS_FATAL_ERROR("Nor LTE eNB neither MmWave eNB neither IAB");
  }
}
      ...
      for (int j = 0; j < nDevs; ++j)
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                // it may be an IAB device
                Ptr<MmWaveIabNetDevice> iabDev = node->GetDevice(j)->GetObject <MmWaveIabNetDevice> ();
                if (iabDev != 0)
                {
                  ueRrc = iabDev->GetBackhaulRrc ();              
                  NS_LOG_LOGIC ("considering IAB IMSI " << iabDev->GetImsi () << " that has cellId " << ueRrc->GetCellId ());
                  if (ueRrc->GetCellId () == m_cellId)
                  {       
