  ...
  if (ueInfo.size () > 0)
    {
      ...
      // NEW APPROACH: Two-phase allocation for IAB nodes
      // Phase 1: Allocate DL for all UEs (including IAB) like regular UEs
      // Phase 2: Allocate UL for all UEs (including IAB) after DL allocation is complete
      
      // Phase 1: DL Allocation for all UEs (including IAB)
      NS_LOG_DEBUG("*** PHASE 1: DL ALLOCATION START ***");
      std::map <uint16_t, struct UeSchedInfo>::iterator itUeInfoDl = itUeInfoStart;
      bool dlPhaseDone = false;
      
      while (!dlPhaseDone) {
        UeSchedInfo &ueSchedInfo = itUeInfoDl->second;
        
        // Allocate DL for all UEs (including IAB) like regular UEs
        if (!ueSchedInfo.m_dlAllocDone && ueSchedInfo.m_maxDlBufSize > 0) {
          ...
          NS_LOG_DEBUG("*** DL PHASE: UE " << itUeInfoDl->first 
          ...
          if (ueSchedInfo.m_dlSymbols > 0 && (nextSymAvail + symPerDlBlock) <= firstUlSymbol) {
            ...
            dci.m_rv = 0;
            dci.m_harqProcess = UpdateDlHarqProcessId(itUeInfoDl->first);
            NS_ASSERT(dci.m_harqProcess < m_phyMacConfig->GetNumHarqProcess());
            NS_LOG_LOGIC("UE " << itUeInfoDl->first << " DL harqId " << (unsigned)dci.m_harqProcess << " HARQ process assigned");

            SlotAllocInfo slotInfo(tempDlslotIdx++, SlotAllocInfo::DL_slotAllocInfo, SlotAllocInfo::CTRL_DATA, SlotAllocInfo::DIGITAL, itUeInfoDl->first, layerIdxDl);
            slotInfo.m_dci = dci;
            ...
            if (m_harqOn == true) {
             ...
             for (unsigned i = 0; i < ueSchedInfo.m_rlcPduInfo.size(); i++) {
               ...
               UpdateDlRlcBufferInfo(itUeInfoDl->first, ueSchedInfo.m_rlcPduInfo[i].m_lcid, ueSchedInfo.m_rlcPduInfo[i].m_size - m_subHdrSize);
        ...
        itUeInfoDl++; // move to next UE
        if (itUeInfoDl == ueInfo.end()) {
          itUeInfoDl = ueInfo.begin();
        }
        if (itUeInfoDl == itUeInfoStart) {
          dlPhaseDone = true;
        }
      }
...
MmWavePaddedHbfMacScheduler::DoIabBackhaulSchedNotify(const struct MmWaveUeMacCschedSapProvider::IabBackhaulSchedInfo& info)
{
  ...
  NS_LOG_DEBUG("MmWavePaddedHbfMacScheduler received DCIs for the backhaul link");
  NS_LOG_DEBUG("*** IAB BACKHAUL NOTIFICATION *** RNTI: " << info.m_dciInfoElementTdma.m_rnti 
