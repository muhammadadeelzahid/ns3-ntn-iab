...
//					cqi = cqi / m_phyMacConfig->GetTotalNumChunk ();
              ...
              NS_LOG_DEBUG("*** BSR PROCESSED *** UE " << itUeInfo->first << " BSR Size: " << ceBsrIt->second << " Max UL Buffer: " << itUeInfo->second.m_maxUlBufSize << " at time " << Simulator::Now().GetSeconds() << "s");
            }
        }
    }
      ...
      while ( ! done )
        {
          ...
          // Debug log for simultaneous DL+UL allocation
          if (itUeInfo->second.m_dlSymbols > 0 && itUeInfo->second.m_ulSymbols > 0) {
            NS_LOG_DEBUG("*** WARNING: SIMULTANEOUS DL+UL ALLOCATION *** UE " << itUeInfo->first 
                          << " DL Symbols: " << (int)itUeInfo->second.m_dlSymbols 
                          << " UL Symbols: " << (int)itUeInfo->second.m_ulSymbols 
                          << " Total: " << (int)(itUeInfo->second.m_dlSymbols + (int)itUeInfo->second.m_ulSymbols) 
                          << " at time " << Simulator::Now().GetSeconds() << "s");
                      }
          
          NS_LOG_DEBUG("*** RESOURCE ASSIGNMENT *** UE " << (int)itUeInfo->first << " DL Symbols: " << (int)itUeInfo->second.m_dlSymbols << " UL Symbols: " << (int)itUeInfo->second.m_ulSymbols << " Total: " << (int)(itUeInfo->second.m_dlSymbols + itUeInfo->second.m_ulSymbols) << " at time " << Simulator::Now().GetSeconds() << "s");
          ...
          if ( ( itUeInfo->second.m_dlSymbols > 0 ) & ( (nextSymAvail + symPerDlBlock) <= firstUlSymbol ) ) // If buffer_size is 0 this UE is not counted in nFlowsDl and no actual block is created
            {
              NS_LOG_DEBUG("*** DL ALLOCATION START *** UE " << itUeInfo->first 
                << " DL Symbols: " << (int)itUeInfo->second.m_dlSymbols 
                << " UL Symbols: " << (int)itUeInfo->second.m_ulSymbols 
                << " at time " << Simulator::Now().GetSeconds() << "s");
              ...
              NS_LOG_DEBUG("*** DL DCI CREATED *** UE " << (int)itUeInfo->first << " Symbols: " << (int)dci.m_symStart << "-" << (int)(dci.m_symStart + dci.m_numSym - 1) << " at time " << Simulator::Now().GetSeconds() << "s");
          ...
          NS_LOG_DEBUG("*** SCHEDULER UL DECISION *** UE " << (int)itUeInfo->first << " UL Symbols: " << (int)ueSchedInfo.m_ulSymbols << " Max UL Symbols: " << (int)itUeInfo->second.m_maxUlSymbols << " Max UL Buffer: " << (int)itUeInfo->second.m_maxUlBufSize << " at time " << Simulator::Now().GetSeconds() << "s");
          if ( ( ueSchedInfo.m_ulSymbols > 0 ) & ( (lastSymAvail - symPerUlBlock + 1 ) >= firstUlSymbol ) )
            {
              NS_LOG_DEBUG("*** UL ALLOCATION START *** UE " << itUeInfo->first 
                << " DL Symbols: " << (int)itUeInfo->second.m_dlSymbols 
                << " UL Symbols: " << (int)itUeInfo->second.m_ulSymbols 
                << " at time " << Simulator::Now().GetSeconds() << "s");
  
              // Check if this UE already has DL allocation (potential conflict)
              if (itUeInfo->second.m_dlSymbols > 0) {
                NS_LOG_DEBUG("*** WARNING: UE " << (int)itUeInfo->first 
                              << " getting BOTH DL and UL allocation!" 
                              << " at time " << Simulator::Now().GetSeconds() << "s");                
              }
              ...
              NS_LOG_DEBUG("*** UL DCI CREATED *** UE " << (int)itUeInfo->first << " Symbols: " << (int)dci.m_symStart << "-" << (int)(dci.m_symStart + dci.m_numSym - 1) << " at time " << Simulator::Now().GetSeconds() << "s");
        ...
        // Summary log for simultaneous DL+UL allocations
        NS_LOG_DEBUG("*** ALLOCATION SUMMARY *** Checking for simultaneous DL+UL allocations at time " << Simulator::Now().GetSeconds() << "s");
        for (auto& uePair : ueInfo) {
          if (uePair.second.m_dlSymbols > 0 && uePair.second.m_ulSymbols > 0) {
            NS_LOG_DEBUG("*** SIMULTANEOUS ALLOCATION DETECTED *** UE " << uePair.first 
                          << " DL: " << (int)uePair.second.m_dlSymbols 
                          << " UL: " << (int)uePair.second.m_ulSymbols 
                          << " at time " << Simulator::Now().GetSeconds() << "s");
          }
        }
