...
MmWavePhyMacCommon::GetTypeId (void)
{
	static TypeId tid = TypeId("ns3::MmWavePhyMacCommon")
			...
			.AddAttribute ("SymbolPerSlot",
						   "Number of symbols per slot -- IAB: not used",
						   UintegerValue (30),
						   MakeUintegerAccessor (&MmWavePhyMacCommon::m_symbolsPerSlot),
						   MakeUintegerChecker<uint32_t> ())
			.AddAttribute ("SymbolsPerSubframe",
							 "OFDM symbols per subframe -- IAB: actually, these are the slots in a subframe",
							 UintegerValue (24),
		   ...
		   .AddAttribute ("CtrlSymbols",
			...
			.AddAttribute ("SlotsPerSubframe",
						   "Number of slots in one subframe -- IAB: not used",
						   UintegerValue (8),
		 ...
		 .AddAttribute ("HarqDlTimeout",
							 "Number of concurrent stop-and-wait Hybrid ARQ processes per user", // TODOIAB update description
							 UintegerValue (20),
