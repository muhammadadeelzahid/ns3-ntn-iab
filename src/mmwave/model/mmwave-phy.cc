  ...
  if (p->PeekPacketTag (tag))
    {
      ...
      NS_LOG_DEBUG("[DEBUG-ENB-PHY] SetMacPdu: frame=" << (int)tag.GetSfn().m_frameNum << ", subframe=" << (int)sfNum << ", slot=" << (int)slotNum << ", layer=" << (int)layerInd << ", packet size=" << p->GetSize() << " bytes" << "device: " << m_netDevice);

          std::map<uint64_t, Ptr<PacketBurst>>::iterator it = m_packetBurstLayerMap.find (tag.Encode());
          if (it == m_packetBurstLayerMap.end ())
            {
              NS_LOG_DEBUG("[DEBUG] SetMacPdu: Creating new packet burst for this slot/layer with key: " << tag.Encode());
          ...
          NS_LOG_DEBUG("[DEBUG] SetMacPdu: Added packet to burst, total packets in burst: " << it->second->GetNPackets());
        }
//    }
  else
    {
...
{
  NS_LOG_DEBUG("*** CONTROL MESSAGE ADDED TO PHY QUEUE *** " << this << " Message ID: " << m->GetMessageId() << " Type: " << m->GetMessageType() << " at time " << Simulator::Now().GetSeconds() << "s");
...
MmWavePhy::SetDlSfAllocInfo (SfAllocInfo sfAllocInfo)
{
  ...
  NS_LOG_DEBUG("*** PHY RECEIVED DL SCHEDULING INFO *** " << this << " Frame: " << sfAllocInfo.m_sfnSf.m_frameNum << " Subframe: " << sfAllocInfo.m_sfnSf.m_sfNum << " Slots: " << sfAllocInfo.m_slotAllocInfo.size() << " at time " << Simulator::Now().GetSeconds() << "s");
...
{
	NS_LOG_DEBUG("Satellite flag set to "<<flag<<" for spectrum and propagation loss model"<<this<< " : "<<m_spectrumPropagationLossModel);	
