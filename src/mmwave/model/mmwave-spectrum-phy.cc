...
MmWaveSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{
	...
	else
	{
		...
		{
			NS_LOG_DEBUG("Processing control message on layer " << m_layerInd << " for cellId " << DlCtrlRxParams->cellId);
			...
			// 	else 
				...
				// else
				// {
					NS_LOG_DEBUG("Starting control message reception for cellId " << DlCtrlRxParams->cellId << " in a " << GetDeviceType() << " dev");
					StartRxCtrl (params);	
				// }
			...
			{
				NS_LOG_DEBUG("Control message received but not in sync with this signal (cellId="
		...
		{
			NS_LOG_DEBUG("Invalid layer index for control message: m_layerInd=" << m_layerInd << ", device=" << this << " device type: " << GetDeviceType());
		}
	}
}
...
{
	NS_LOG_DEBUG("StartRxData called on device " << this << " slot index: " << (int) params->slotInd << ", devicetype "<< GetDeviceType() << ", m_layerInd=" << m_layerInd << ", cellId=" << params->cellId << ", m_cellId=" << m_cellId);
...
MmWaveSpectrumPhy::StartTxDataFrames (Ptr<PacketBurst> pb, std::list<Ptr<MmWaveControlMessage> > ctrlMsgList, Time duration, uint8_t slotInd)
{
  ...
  case RX_CTRL:
	...
	case IDLE:
	{
		...
		{
			NS_LOG_DEBUG("ctrlMsgList is not empty");
		}
