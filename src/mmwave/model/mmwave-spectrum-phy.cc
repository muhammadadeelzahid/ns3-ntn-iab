...
void
MmWaveSpectrumPhy::LogPacketActivity(const std::string& packetType, 
									 const std::string& fromInfo, 
									 const std::string& toInfo, 
									 double sumTxPsd, 
									 const Ptr<const SpectrumValue>& txPsd,
									 double sumRxPsd,
									 const Ptr<const SpectrumValue>& rxPsd)
{
	std::ofstream logFile("packet_activity_log.txt", std::ios_base::app);
	if (!logFile.is_open())
	{
		NS_LOG_ERROR("Cannot open packet activity log file");
		return;
	}
	
	double currentTime = Simulator::Now().GetSeconds();
	
	logFile << "Time: " << currentTime << "s, ";
	logFile << "Packet Type: " << packetType << ", ";
	logFile << "From: " << fromInfo << ", ";
	logFile << "To: " << toInfo << ", ";
	logFile << "Sum(TxPSD): " << sumTxPsd << ", ";
	logFile << "Sum(RxPSD): " << sumRxPsd << ", ";
	logFile << "Device: " << this << ", ";
	logFile << "Layer: " << (int)m_layerInd;
	
	// Log TxPSD values if available
	if (txPsd)
	{
		logFile << ", TxPSD: [";
		for (Values::const_iterator it = txPsd->ConstValuesBegin(); it != txPsd->ConstValuesEnd(); ++it)
		{
			logFile << *it;
			if (std::next(it) != txPsd->ConstValuesEnd())
			{
				logFile << ", ";
			}
		}
	...
	// Log RxPSD values if available
	if (rxPsd)
	{
		logFile << ", RxPSD: [";
		for (Values::const_iterator it = rxPsd->ConstValuesBegin(); it != rxPsd->ConstValuesEnd(); ++it)
		{
			logFile << *it;
			if (std::next(it) != rxPsd->ConstValuesEnd())
			{
				logFile << ", ";
			}
		}
...
MmWaveSpectrumPhy::SetDevice(Ptr<NetDevice> d)
{
	...
	Ptr<MmWaveIabNetDevice> iabNetDev = 
			DynamicCast<MmWaveIabNetDevice> (GetDevice ());
	Ptr<MmWaveUeNetDevice> ueNetDev = 
			DynamicCast<MmWaveUeNetDevice> (GetDevice ());
	Ptr<McUeNetDevice> mcNetDev = 
...
{
  NS_LOG_LOGIC (this << " state: " << m_state << " -> " << newState << "device type: " << GetDeviceType() << " device: " << this);
  m_state = newState;
}
								  ...
								  uint8_t txLayerInd, uint8_t rxLayerInd)
{
	...
	ExpectedTbInfo_t tbInfo = {ndi, size, mcs, chunkMap, harqId, rv, 0.0, downlink, false, false, 0, symStart, numSym, txLayerInd, rxLayerInd};
...
MmWaveSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> params)
{
	...
	NS_LOG_DEBUG("StartRx StartRx Device Type: " << GetDeviceType()<<" Device: " << this<<" m_layerInd: "<<(int)m_layerInd);
	...
	Ptr<McUeNetDevice> McUeTx =	
			DynamicCast<McUeNetDevice> (params->txPhy->GetDevice ());
	Ptr<MmWaveIabNetDevice> iabTx = 
				DynamicCast<MmWaveIabNetDevice> (params->txPhy->GetDevice ());

	if(GetDeviceType() == ENB && EnbTx != 0)
	{
		// eNB to eNB, skip
		NS_LOG_INFO ("BS to BS transmission neglected.");
		return;
	}
	else if(GetDeviceType() == UE && UeTx != 0)
	{
		// UE to UE, skip
		NS_LOG_INFO ("UE to UE transmission neglected.");
		return;
	}
	...
	{
		NS_LOG_DEBUG("Data message received");
		...
		if(GetDeviceType() == UE)
		{
			...
			if (ueRx != 0)
			{
				NS_LOG_DEBUG("StartRx UE's allocated layer index:" << (int) ueRx->GetPhy ()->GetAllocLayerInd () << ", current layer:" << m_layerInd);
			}

			if ((ueRx!=0) && (ueRx->GetPhy ()->IsReceptionEnabled () == false))
			{	// if the first cast is 0 (the device is MC) then this if will not be executed
				NS_LOG_DEBUG("StartRx UE's reception is disabled");
				isAllocated = false;
			} 

			if ((ueRx != 0) && (ueRx->GetPhy ()->GetAllocLayerInd() != layerInd))
			{               
			  NS_LOG_DEBUG ("This data is not for this UE");
			  isMyLayer = false;
			}
		...
		else if(GetDeviceType() == MCUE)
		{
			...
			{	// this is executed if the device is MC and is transmitting
				NS_LOG_DEBUG("StartRx MC's reception is disabled");
				isAllocated = false;
			}

			if ((rxMcUe != 0) && (rxMcUe->GetMmWavePhy ()->GetAllocLayerInd() != layerInd))
			{               
				NS_LOG_DEBUG("StartRx MC's layer index is not the same as the received layer index");
				isMyLayer = false;
			}
		}
		else if(GetDeviceType() == IAB)
		{
			// TODOIAB if the backhaul is transmitting, then set isAllocated to false
			...
			if(rxIabDev != 0)
			{
				...
				// IAB handling to check for layerindex
				if (rxIabDev->GetBackhaulPhy()->GetAllocLayerInd() != layerInd)
				{
					NS_LOG_DEBUG("StartRx IAB's layer index is not the same as the received layer index");
					isMyLayer = false;
				}
				else if (GetAccessSpectrumPhy())
				{
					Ptr<MmWaveEnbPhy> accessEnbPhy = rxIabDev->GetAccessPhy();
					uint8_t numAllocLayers = accessEnbPhy->GetCurrNumAllocLayers();
				
					if (layerInd >= numAllocLayers)
					{
						isMyLayer = false;
						NS_LOG_INFO("Access PHY: incoming layer " << (int)layerInd << " not allocated in ENB PHY (only " << (int)numAllocLayers << " layers active)");
					}
				}
		...
		else if (GetDeviceType() == ENB)
		{
			...
			if ((enbRx != 0) && (enbRx->GetPhy (m_layerInd)->IsReceptionEnabled () == false))
			{
				isAllocated = false;
			}
		...
		NS_LOG_INFO("RXData signal layer index: " << (int)layerInd << ", cell ID: " << (int)cellId << ", duration: " << duration
				...
				<< " RNTI: "<< (int )bearerTag.GetRnti ());
  
		// Add detailed RNTI logging for debugging
		uint16_t rnti = bearerTag.GetRnti();
		std::string deviceTypeStr;
		switch(GetDeviceType())
		{
			case ENB: deviceTypeStr = "ENB"; break;
			case IAB: deviceTypeStr = "IAB"; break;
			case UE: deviceTypeStr = "UE"; break;
			case MCUE: deviceTypeStr = "MCUE"; break;
			default: deviceTypeStr = "UNKNOWN"; break;
		}
		NS_LOG_DEBUG("[DEBUG-RX] Device " << this << " (type: " << deviceTypeStr << ") receiving from RNTI: " << rnti << " at time " << Simulator::Now().GetSeconds());
  
		// Log packet reception activity for data frames
		std::string fromInfo = "node_id=" + std::to_string(mmwaveDataRxParams->txPhy->GetDevice()->GetNode()->GetId()) + 
							  "/cell_id=" + std::to_string(cellId) + 
							  "/imsi=" + std::to_string(rnti);
		std::string toInfo = "node_id=" + std::to_string(GetDevice()->GetNode()->GetId()) + 
							"/cell_id=" + std::to_string(m_cellId) + 
							"/imsi=" + std::to_string(rnti);
		double sumRxPsd = Sum(*(mmwaveDataRxParams->psd));
		LogPacketActivity("Data", fromInfo, toInfo, 0.0, nullptr, sumRxPsd, mmwaveDataRxParams->psd);
  
		NS_LOG_DEBUG("isAllocated " << isAllocated);
		if (isAllocated)
		{

			// Remove PSD calculation from StartTx
			Ptr<MobilityModel> txMobility = mmwaveDataRxParams->txPhy->GetMobility ();
			Ptr<MmWave3gppChannel> pathlossmodel = DynamicCast<MmWave3gppChannel>(m_channel->GetSpectrumPropagationLossModel());
			if (GetDeviceType() == ENB)
			{//in the BS Rx case, beamforming is evaluated using the layer of this mmwave-spectrum-phy, not the transmission layer index
				...
				NS_LOG_DEBUG("Computed the BF gain at BS receiving in layer " << (int ) m_layerInd << " SpectrumPhy::StartRx for signal of layer " << (int) layerInd << " power: " << Sum (*(mmwaveDataRxParams->psd)));
			}
			else if (GetDeviceType() == UE)
			{
				// UE RX
				Ptr<MmWaveUeNetDevice> ueRx = DynamicCast<MmWaveUeNetDevice> (GetDevice ());
				...
				NS_LOG_DEBUG("Computed the BF gain at UE allocated layer " << (int ) ueRx->GetPhy ()->GetAllocLayerInd() << " SpectrumPhy::StartRx for signal of layer " << (int) layerInd << " the UE m_layerInd is "<<(int)  m_layerInd);
			}
			else if (GetDeviceType() == IAB)
			{
				Ptr<MmWaveIabNetDevice> iabRx = DynamicCast<MmWaveIabNetDevice>(GetDevice());
				NS_ASSERT(iabRx != nullptr);

				if (GetAccessSpectrumPhy())
				{
					// IAB Access: behaves like an ENB receiving from a UE (UL)
					mmwaveDataRxParams->psd = pathlossmodel->CalcRxPowerSpectralDensityMultiLayers(
						mmwaveDataRxParams->psd,
						txMobility,
						this->GetMobility(),
						0,               // txLayerInd is from UE (always 0)
						m_layerInd       // receiving IAB-ENB layer
					);
					NS_LOG_DEBUG("Computed BF gain at IAB Access (ENB part) receiving on layer " << (int)m_layerInd
								<< " for signal of layer " << (int)layerInd << " signal power: " << Sum (*(mmwaveDataRxParams->psd)));
				}
				else
				{
					// IAB Backhaul: behaves like a UE receiving from donor gNB
					mmwaveDataRxParams->psd = pathlossmodel->CalcRxPowerSpectralDensityMultiLayers(
						mmwaveDataRxParams->psd,
						txMobility,
						this->GetMobility(),
						layerInd,        // txLayerInd from donor ENB
						m_layerInd       // receiving IAB-UE layer (usually 0)
					);
					NS_LOG_DEBUG("Computed BF gain at IAB Backhaul (UE part) m_layerInd = " << (int)m_layerInd
								<< " receiving signal of layer " << (int)layerInd << " signal power: " << Sum (*(mmwaveDataRxParams->psd)));
				}
			}

			NS_LOG_DEBUG(this<<" Node "<<  GetDevice()->GetAddress() <<" detected in layer " << (int) m_layerInd << " a signal with power "<<Sum (*(mmwaveDataRxParams->psd)));
			...
			{
				NS_LOG_DEBUG("cellID from mmwaveDataRxParams: " << mmwaveDataRxParams->cellId << " m_cellId: " << m_cellId);
				NS_LOG_DEBUG("isMyLayer: " << isMyLayer);
			...
			{
				NS_LOG_DEBUG("Data is not for this UE/Layer, StartRxData");
			}
			/*
			else
			{
	...
	else
	{
		...
		{
			NS_LOG_DEBUG("[DEBUG-CONTROL] Processing control message on layer " << m_layerInd << " for cellId " << DlCtrlRxParams->cellId);
			...
			// 	else 
				...
				// Log packet reception activity for control frames
				std::string fromInfo = "node_id=" + std::to_string(DlCtrlRxParams->txPhy->GetDevice()->GetNode()->GetId()) + 
									  "/cell_id=" + std::to_string(DlCtrlRxParams->cellId) + 
									  "/imsi=0"; // Control frames don't have specific IMSI
				std::string toInfo = "node_id=" + std::to_string(GetDevice()->GetNode()->GetId()) + 
									"/cell_id=" + std::to_string(m_cellId) + 
									"/imsi=0"; // Control frames don't have specific IMSI
				double sumRxPsd = Sum(*(DlCtrlRxParams->psd));
				LogPacketActivity("Control", fromInfo, toInfo, 0.0, nullptr, sumRxPsd, DlCtrlRxParams->psd);

				// else
				// {
					NS_LOG_DEBUG("[DEBUG-CONTROL] Starting control message reception for cellId " << DlCtrlRxParams->cellId << " in a " << GetDeviceType() << " dev");
					StartRxCtrl (params);	
				// }
			...
			{
				NS_LOG_DEBUG("[DEBUG-CONTROL] Control message received but not in sync with this signal (cellId="
		...
		{
			NS_LOG_DEBUG("[DEBUG-CONTROL] Invalid layer index for control message: m_layerInd=" << m_layerInd << ", device=" << this << " device type: " << GetDeviceType());
		}
	}
}
...
{
	NS_LOG_DEBUG("[DEBUG] StartRxData called on device " << this << " slot index: " << (int) params->slotInd << ", devicetype "<< GetDeviceType() << ", m_layerInd=" << m_layerInd << ", cellId=" << params->cellId << ", m_cellId=" << m_cellId);
...
MmWaveSpectrumPhy::StartRxCtrl (Ptr<SpectrumSignalParameters> params)
{
	...
	switch (m_state)
	{
		...
		case IDLE:
		{
			...
			NS_LOG_DEBUG("StartRxCtrl called on device ");
			if (cellId  == m_cellId)
			{
				NS_LOG_DEBUG("cell ID found");
				if(m_state == RX_CTRL)
				{
					NS_LOG_DEBUG("state is RX_CTRL");
					Ptr<MmWaveUeNetDevice> ueRx =
									DynamicCast<MmWaveUeNetDevice> (GetDevice ());
					Ptr<McUeNetDevice> rxMcUe =
									DynamicCast<McUeNetDevice> (GetDevice ());
					Ptr<MmWaveIabNetDevice> rxIab = 
									DynamicCast<MmWaveIabNetDevice> (GetDevice ());
				...
				NS_LOG_DEBUG (this << " synchronized with this signal (cellId=" << cellId << ")");
				if (m_state == IDLE)
				{
					// first transmission, i.e., we're IDLE and we start RX
					...
					m_firstRxStart = Simulator::Now ();
					m_firstRxDuration = params->duration;
					NS_LOG_DEBUG (this << " scheduling EndRx with delay " << params->duration);
					// store the DCIs
			...
			{
				NS_LOG_DEBUG("StartRxCtrl Invalid layer index for control message: m_layerInd=" << m_layerInd << ", device=" << this << ", cellId=" << cellId << ", m_cellId=" << m_cellId << ", state=" << m_state);
				// Do nothing
			}
			break;
	}
...
{	
	NS_LOG_DEBUG("EndRxData called on device " << this << ", m_layerInd=" << m_layerInd);
	m_interferenceData->EndRx();

	double sinrAvg = Sum(m_sinrPerceived)/(m_sinrPerceived.GetSpectrumModel()->GetNumBands());
	NS_LOG_DEBUG("sinrAvg: " << sinrAvg << " m_sinrPerceived: " << m_sinrPerceived << " num bands: " << m_sinrPerceived.GetSpectrumModel()->GetNumBands());
	...
	while (itTb != m_expectedTbs.end ())
	{
		if ((m_dataErrorModelEnabled)&&(m_rxPacketBurstList.size ()>0))
		{
			...
			{
				NS_LOG_INFO ("EndRxData Corrupted: " << this << " RNTI " << itTb->first << " size " << itTb->second.size << " mcs " << (uint32_t)itTb->second.mcs << " bitmap " << itTb->second.rbBitmap.size () << " rv " << rv << " TBLER " << tbStats.tbler << " corrupted " << itTb->second.corrupt);
			}
		}
		itTb++;
	}
			...
			i != m_rxPacketBurstList.end (); ++i)
	{
		for (std::list<Ptr<Packet> >::const_iterator j = (*i)->Begin (); j != (*i)->End (); ++j)
		{
			...
			NS_LOG_DEBUG("EndRxData, packet was for rnti: " << rnti<<" m_cellId: " << m_cellId<<" getdevicetype: " << GetDeviceType()<<" layerInd: " << m_layerInd<<" this: " << this);
			if(itTb != m_expectedTbs.end ())
			{
				...
				if (enbRx)
				{
					...
					NS_LOG_DEBUG("EndRxData enbRx UL\t" << "frameNum: " << traceParams.m_frameNum << "\t" << "sfNum: " << (unsigned)traceParams.m_sfNum << "\t" << "symStart: " << (unsigned)traceParams.m_symStart << "\t" << "numSym: " << (unsigned)traceParams.m_numSym << "\t" << "cellId: " << traceParams.m_cellId << "\t" <<"Rnti: "<< traceParams.m_rnti << "\t" <<"tbSize: " << traceParams.m_tbSize << "\t" << "mcs: " << (unsigned)traceParams.m_mcs << "\t" << "rv: " << (unsigned)traceParams.m_rv << "\t" << "sinr: " << 10*std::log10(traceParams.m_sinr) << "\t" << "corrupt: " << traceParams.m_corrupt << "\t" << "tbler: " <<  traceParams.m_tbler);		
				...
				else if (ueRx)
				{
					...
					NS_LOG_DEBUG("EndRxData ueRx DL\t" << "frameNum: " << traceParams.m_frameNum << "\t" << "sfNum: " << (unsigned)traceParams.m_sfNum << "\t" << "symStart: " << (unsigned)traceParams.m_symStart
					<< "\t" << "numSym: " << (unsigned)traceParams.m_numSym << "\t" << "cellId: " << traceParams.m_cellId
					<< "\t" <<"Rnti: "<< traceParams.m_rnti << "\t" <<"tbSize: " << traceParams.m_tbSize << "\t" << "mcs: " << (unsigned)traceParams.m_mcs << "\t" << "rv: " << (unsigned)traceParams.m_rv << "\t"
					<< "sinr: " << 10*std::log10(traceParams.m_sinr) << "\t" << "corrupt: " << traceParams.m_corrupt << "\t" << "tbler: " <<  traceParams.m_tbler);
					if(DynamicCast<MmWaveEnbNetDevice>(ueRx->GetTargetEnb()) != 0)
				...
				else if (rxMcUe)
				{
					...
					NS_LOG_DEBUG("EndRxData rxMcUe DL\t" << "frameNum: " << traceParams.m_frameNum << "\t" << "sfNum: " << (unsigned)traceParams.m_sfNum << "\t" << "symStart: " << (unsigned)traceParams.m_symStart
					<< "\t" << "numSym: " << (unsigned)traceParams.m_numSym << "\t" << "cellId: " << traceParams.m_cellId
					<< "\t" <<"Rnti: "<< traceParams.m_rnti << "\t" <<"tbSize: " << traceParams.m_tbSize << "\t" << "mcs: " << (unsigned)traceParams.m_mcs << "\t" << "rv: " << (unsigned)traceParams.m_rv << "\t"
					<< "sinr: " << 10*std::log10(traceParams.m_sinr) << "\t" << "corrupt: " << traceParams.m_corrupt << "\t" << "tbler: " <<  traceParams.m_tbler);
				...
				{
					NS_LOG_DEBUG("EndRxData iabRx DL\t" << "frameNum: " << traceParams.m_frameNum << "\t" << "sfNum: " << (unsigned)traceParams.m_sfNum << "\t" << "symStart: " << (unsigned)traceParams.m_symStart
					<< "\t" << "numSym: " << (unsigned)traceParams.m_numSym << "\t" << "cellId: " << traceParams.m_cellId
					<< "\t" <<"Rnti: "<< traceParams.m_rnti << "\t" <<"tbSize: " << traceParams.m_tbSize << "\t" << "mcs: " << (unsigned)traceParams.m_mcs << "\t" << "rv: " << (unsigned)traceParams.m_rv << "\t"
					<< "sinr: " << 10*std::log10(traceParams.m_sinr) << "\t" << "corrupt: " << traceParams.m_corrupt << "\t" << "tbler: " <<  traceParams.m_tbler);
				...
				if (!itTb->second.harqFeedbackSent)
				{
					...
					if (!itTb->second.downlink)  // UPLINK TB
					{
						...
						if (itTb->second.corrupt)
						{
							...
							NS_LOG_DEBUG ("EndRxData Corrupted UE" << rnti << " send UL-HARQ-NACK" << " harqId " << (unsigned)itTb->second.harqProcessId <<
...
//														" mi " << itTb->second.mi << " tbler " << itTb->second.tbler << " SINRavg " << sinrAvg);
					...
					else
					{
						...
						if (itHarq==harqDlInfoMap.end ())
						{
							...
							{
								harqDlInfo.m_harqStatus = DlHarqInfo::NACK;
								NS_LOG_DEBUG ("EndRxData Corrupted UE" << rnti << " send DL-HARQ-NACK" << " harqId " << (unsigned)itTb->second.harqProcessId <<
															" size " << itTb->second.size << " mcs " << (unsigned)itTb->second.mcs <<
															" mi " << itTb->second.mi << " tbler " << itTb->second.tbler << " SINRavg " << sinrAvg);
...
//								              " mi " << itTb->second.mi << " tbler " << itTb->second.tbler << " SINRavg " << sinrAvg);
			...
			{
				NS_LOG_DEBUG("EndRxData Packet is not for this UE/Layer, EndRxData, Packet not found in tbMap, packet was for rnti: " << rnti<<" my cellId: " << m_cellId<<" getdevicetype: " << GetDeviceType()<<" layerInd: " << m_layerInd<<" this: " << this);
	...
	{
		NS_LOG_DEBUG("EndRxData calling the callback for message received");
		
		// Log control messages being received with data frames
		std::list<Ptr<MmWaveControlMessage> >::iterator it;
		int msgCount = 1;
		for (it = m_rxControlMessageList.begin (); it != m_rxControlMessageList.end (); it++)
		{
			Ptr<MmWaveControlMessage> msg = (*it);
			NS_LOG_INFO("SpectrumPhy " << this << " received control message with data frame - type " << msg->GetMessageType() << " Message ID: " << msg->GetMessageId() << " Message Number: " << msgCount);
			msgCount++;
		}
...
{
	NS_LOG_FUNCTION(this);
	NS_LOG_DEBUG("EndRxCtrl called on device " << this << ", m_layerInd=" << m_layerInd);
  ...
  if (!m_rxControlMessageList.empty ())
    {
		NS_LOG_DEBUG("m_rxControlMessageList is not empty");
		
		// Log control messages being received with their IDs
		std::list<Ptr<MmWaveControlMessage> >::iterator it;
		int msgCount = 1;
		for (it = m_rxControlMessageList.begin (); it != m_rxControlMessageList.end (); it++)
		{
			Ptr<MmWaveControlMessage> msg = (*it);
			NS_LOG_INFO("SpectrumPhy " << this << " received control message type " << msg->GetMessageType() << " Message ID: " << msg->GetMessageId() << " Message Number: " << msgCount);
			msgCount++;
		}
		
      if (!m_phyRxCtrlEndOkCallback.IsNull ())
        {
			NS_LOG_DEBUG("EndRxCtrl calling the callback for message received");
          m_phyRxCtrlEndOkCallback (m_rxControlMessageList);
        }
    }
...
{
	NS_LOG_DEBUG("StartTxDataFrames called on device " << this << ", m_layerInd=" << m_layerInd);
	switch (m_state)
	{
	case RX_DATA:
  case RX_CTRL:
	...
	case IDLE:
	{
		...
		// Log control messages being transmitted with data frames
		if (!ctrlMsgList.empty())
		{
			std::list<Ptr<MmWaveControlMessage> >::iterator it;
			int msgCount = 1;
			...
			{
				Ptr<MmWaveControlMessage> msg = (*it);
				NS_LOG_INFO("SpectrumPhy " << this << " transmitting control message with data frame - type " << msg->GetMessageType() << " Message ID: " << msg->GetMessageId() << " Message Number: " << msgCount);
				msgCount++;
			}
		}
        ...
        NS_LOG_INFO ("StartTxDataFrames Data transmission slot "<< (int ) slotInd <<" duration "<< duration <<" with layer " << (int ) macTag.GetLayerInd ());
		...
		NS_LOG_DEBUG("StartTxDataFrames: to layerInd " << (int)txParams->layerInd);
		
		// Add RNTI logging for debugging uplink transmissions
		if (pb && !pb->GetPackets().empty())
		{
			LteRadioBearerTag bearerTag;
			if ((*pb->GetPackets().begin())->PeekPacketTag(bearerTag))
			{
				uint16_t rnti = bearerTag.GetRnti();
				std::string deviceTypeStr;
				switch(GetDeviceType())
				...
				NS_LOG_DEBUG("StartTxDataFrames ThisDevice " << this << " (type: " << deviceTypeStr << ") transmitting for RNTI: " << rnti << "for cellId: " << txParams->cellId << " for layerInd: " << (int)txParams->layerInd << " at time " << Simulator::Now().GetSeconds());
				
				// Log packet transmission activity
				std::string fromInfo = "node_id=" + std::to_string(GetDevice()->GetNode()->GetId()) + 
									  "/cell_id=" + std::to_string(m_cellId) + 
									  "/imsi=" + std::to_string(rnti);
				std::string toInfo = "cell_id=" + std::to_string(txParams->cellId) + 
									"/layer=" + std::to_string((int)txParams->layerInd);
				double sumTxPsd = Sum(*m_txPsd);
				LogPacketActivity("Data", fromInfo, toInfo, sumTxPsd, m_txPsd, 0.0, nullptr);
			}
		}
...
MmWaveSpectrumPhy::StartTxDlControlFrames (std::list<Ptr<MmWaveControlMessage> > ctrlMsgList, Time duration)
{
  ...
  case RX_DATA:
	...
	case IDLE:
	{
		...
		// Log control messages being transmitted with their IDs
		std::list<Ptr<MmWaveControlMessage> >::iterator it;
		int msgCount = 1;
		for (it = ctrlMsgList.begin (); it != ctrlMsgList.end (); it++)
		{
			Ptr<MmWaveControlMessage> msg = (*it);
			NS_LOG_INFO("SpectrumPhy " << this << " transmitting control message type " << msg->GetMessageType() << " Message ID: " << msg->GetMessageId() << " Message Number: " << msgCount);
			msgCount++;
		}
		...
		// Log packet transmission activity for control frames
		std::string fromInfo = "node_id=" + std::to_string(GetDevice()->GetNode()->GetId()) + 
							  "/cell_id=" + std::to_string(m_cellId) + 
							  "/imsi=0"; // Control frames don't have specific IMSI
		std::string toInfo = "cell_id=" + std::to_string(txParams->cellId) + 
							"/broadcast"; // Control frames are broadcast
		double sumTxPsd = Sum(*m_txPsd);
		LogPacketActivity("Control", fromInfo, toInfo, sumTxPsd, m_txPsd, 0.0, nullptr);
		
		m_channel->StartTx (txParams);
		m_endTxEvent = Simulator::Schedule (duration, &MmWaveSpectrumPhy::EndTx, this);
		NS_LOG_DEBUG("StartTxDlControlFrames called on device " << this << " m_layerInd=" << m_layerInd << " state: " << m_state << " Tx to cellId " << txParams->cellId << " m_cellID " << m_cellId);
	}
	}
	return false;
}
