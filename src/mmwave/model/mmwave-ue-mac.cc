...
{
	NS_LOG_FUNCTION (this);
	NS_LOG_DEBUG("DoTransmitPdu called: this=" << this << ", RNTI=" << params.rnti << ", LCID=" << (uint32_t)params.lcid << ", size=" << params.pdu->GetSize() << ", harqProcessId=" << (uint32_t)params.harqProcessId);
    ...
    case (MmWaveControlMessage::DCI_TDMA):
      {
        ...
        if(m_iabBackhaulSapProvider != 0)
        {
          ...
          NS_LOG_INFO("UE MAC " << m_rnti << " notifying backhaul scheduler with message ID: " << iabInfo.m_messageId 
                      << " Target Frame: " << sfn.m_frameNum << " Target Subframe: " << (int)sfn.m_sfNum);
